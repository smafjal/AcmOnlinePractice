...
void Dijkstra(int s,int id)
{
    ...
    while(!Q.empty())
    {
        pq tmp=Q.top();Q.pop();
        int u=tmp.node;
        int precost=tmp.weight;
//        if(precost>dis[id][u]) continue;
...
int main()
{
    ...
    while(t--)
    {
        ...
        FOR(i,0,2)FOR(j,0,N+1) Clr(adj[i][j]);
        edge.clear();
        ...
        FOR(i,0,M)
        {
            ...
            int v=edge[i].sc.fs;
            int w=edge[i].sc.sc;
//            deb(dis[0][u],w,dis[1][v]);lookalike;

            if(dis[0][u]+w+dis[1][v]<=P) mx=max(mx,w);
        }
        printf("Case %d: %d\n",++cas,mx);
    }
    return 0;
