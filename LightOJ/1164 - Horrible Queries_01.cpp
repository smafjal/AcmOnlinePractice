...
void refresh(ll nod,ll left,ll right)
{
    if(left==right)
    {
        tree[nod]+=col[nod];
    ...
    else
    {
        col[nod*2]+=col[nod];
        col[nod*2+1]+=col[nod];
        tree[nod]=tree[nod]+(right-left+1)*col[nod];
    }
    col[nod]=0;
    return;
}
...
void update(ll nod,ll left,ll right,ll i,ll j,ll val)
{
    ...
    if(col[nod*2]) refresh(nod*2,left,mid);
    if(col[nod*2+1]) refresh(nod*2+1,mid+1,right);
...
int main()
{
    ...
    {
        scanf("%lld %lld",&N,&Q);
        for(int i=0;i<=4*N;i++){col[i]=tree[i]=0;}
