...
bool findpath(int a,int b)
{
    ...
    FOR(i,0,N+M+2)
    {
        if(grid[a][i]>0 and findpath(i,b))
        {
            grid[a][i]--;
            grid[i][a]++;
            return true;
...
//    deb(N,M);
    ...
    FOR(i,0,N)
    {
        ...
        {
            int mod=A[N+j]%A[i];
            if(mod==0) grid[i][N+j]=1;
        }
    }

    int S=N+M;
    int T=N+M+1;
    FOR(i,0,N) grid[S][i]=1;
    FOR(i,0,M) grid[N+i][T]=1;

    FOR(i,0,T+1)FOR(j,0,T+1) G[i][j]=grid[i][j];

    mem(col,0);
    while(findpath(S,T)) mem(col,0);
...
int main()
{
    ...
    while(t--)
    {
        ...
        scanf("%d",&M);
        FOR(i,0,M) scanf("%lld",A+(N+i));

        int cnt=find_max_matching();
        printf("Case %d: %d\n",++cas,cnt);
    }
