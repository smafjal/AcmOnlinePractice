...
ll tree[4*MAX],col[4*MAX];
ll N,Q;

void refresh(int nod,int left,int right)
{
    if(left==right) tree[nod]=col[nod];
    else
    {
        col[nod*2]=col[nod];
        col[nod*2+1]=col[nod];
        tree[nod]=(right-left+1)*col[nod];
    }
    col[nod]=-1;
    return;
}

void update(int nod,int left,int right,int i,int j,int val)
{
    if(col[nod]!=-1) refresh(nod,left,right);
    ...
    if(col[nod*2]!=-1) refresh(nod*2,left,mid);
    if(col[nod*2+1]!=-1) refresh(nod*2+1,mid+1,right);
    tree[nod]=tree[nod*2]+tree[nod*2+1];
    return;
}

ll quary(int nod,int left,int right,int i,int j)
{
    if(col[nod]!=-1) refresh(nod,left,right);
