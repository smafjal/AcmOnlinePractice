...
Given an array with N elements, indexed from 1 to N. Now you will be given some queries in the form I J,
your task is to find the minimum value from index I to J.
...
{
    if(right<i or left>j) return oo;
    if(left>=i and right<=j) return  tree[nod];

    ll mid=(left+right)/2;
    int p=query(nod*2,left,mid,i,j);
    int q=query(nod*2+1,mid+1,right,i,j);
    return min(p,q);
}
