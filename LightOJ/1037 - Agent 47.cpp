...
ll rec(int mask)
{
    ...
    for(int i=0;i<N;i++)
    {
        if((mask&(1<<i))==0)
        {
            for(int j=0;j<N;j++)
            {
                if(arr[i][j] and (mask&(1<<j))!=0)
                {
                    if(health[j]%arr[i][j]) cost=health[j]/arr[i][j]+1;
                    else cost=health[j]/arr[i][j];
                    ret = min(ret,rec(mask^(1<<j))+cost);
                }
                if((mask&(1<<j))!=0)ret=min(ret,rec(mask^(1<<j))+health[j]);
            }
        }
        else ret=min(ret,rec(mask^(1<<i))+health[i]);
    }
...
int main()
{
    ...
    {
        scanf("%lld",&N);
        for(int i=0;i<N;i++) scanf("%lld",&health[i]);
        char c;
        for(int i=0;i<N;i++)
        for(int j=0;j<N;j++)
        {
        ...
        {
            int cost = health[i];
            ans=min((ll)ans,rec(((1<<N)-1)^(1<<i))+cost);
        }
        printf("Case %d: %lld\n",++cas,ans);
    }
    return 0;
