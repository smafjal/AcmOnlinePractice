...
int KMP(int x1,int x2)
{
    string A=store[x2]+"#"+store[x1];
    int k=0;par[0]=0;
    int len=SZ(store[x2]);

    FOR(i,1,SZ(A))
    {
        while((k>0) && (A[i]!=A[k])) k=par[k-1];
        if(A[i]==A[k]) k++;
        par[i]=k;
        if(k==len)return 0;
    }
...
int dp[MAX][(1<<15)+ 10],border;

int rec(int cur,int mask)
{
    if(mask==border) return 0;
    int &ret=dp[cur][mask];
    if(ret!=-1) return ret;
    ret=oo;
    ...
    for(int i=0;i<N;i++)
    {
        if(!Check(mask,i))
        {
            if(G[cur][i]) ret=min(ret,rec(i,Set(mask,i))+G[cur][i]);
            else ret=min(ret,rec(cur,Set(mask,i)));
        }
    }
    return ret;
}
...
void print(int cur,int mask)
{
    if(mask==border) return;
    int &ret=dp[cur][mask];

    string ans="ZZZZZ";int idx,next,start;
    ...
    for(int i=0;i<N;i++)
    {
        if(!Check(mask,i))
        {
            ...
            if(ret==rec(next,Set(mask,i))+cost)
            {
                string tmp=store[i].substr(SZ(store[i])-cost);
                if(tmp<ans)
                {
                    ans=tmp;
                    start=next;
...
int main()
{
    ...
    while(t--)
    {
        ...
        FOR(i,0,N)
        {
            cnt=rec(i,(1<<i))+SZ(store[i]);
            if(cnt<=ans)
            {
                if(cnt==ans)
                {
                    if(store[i]<store[idx])
                        idx=i;
                }
                else
                {
