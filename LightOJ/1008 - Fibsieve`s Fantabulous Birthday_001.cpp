...
void worke1(ll row,ll cul,ll r,ll num,ll cas)
{
    ...
    if(num==r) {printf("Case %lld: %lld %lld\n",cas,row,(ll)1);return;}
    if(num==(cul*cul)){printf("Case %lld: %lld %lld\n",cas,(ll)1,cul);return;}
    cul++;
    ll up=(r-cul)+1;
    if(num<r && num>=up)
    {
...
int main()
{
    ...
    {
        cas++;
        ll num,row,cul,r,val,c=0,tmp;
        scanf("%lld",&num);
        if(num==1) {printf("Case %lld: %lld %lld\n",cas,(ll)1,(ll)1);continue;}
        if(num==2) {printf("Case %lld: %lld %lld\n",cas,(ll)1,(ll)2);continue;}
        r=sqrt(num);r=r*r;
