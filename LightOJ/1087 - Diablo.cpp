...
void cut(int node,int left,int right,int pos)
{
    if(left==right)
    {
        ans=tree[node].value;
        tree[node].value=-1;tree[node].num=0;
        return;
    }
    int mid=(left+right)/2;
    int p=tree[node*2].num;
    int q=tree[node*2+1].num;
    if(p>=pos) cut(node*2,left,mid,pos);
    else cut(node*2+1,mid+1,right,pos-p);
    tree[node].num = tree[node*2].num+tree[node*2+1].num;
    return;
}

void insert(int node,int left,int right,int idx,int val)
{
    if(left==right){tree[node].value=val;tree[node].num=1;return;}
    int mid=(left+right)/2;
    if(idx<=mid) insert(node*2,left,mid,idx,val);
    else insert(node*2+1,mid+1,right,idx,val);
    tree[node].num=tree[node*2].num+tree[node*2+1].num;
    return;
}
...
int main()
{
    int cas=0;int t;int x,y;char c;
    scanf("%d",&t);
