...
int rec(int mask)
{
    ...
    ret=oo;int i,j;
    for(i=0;i<N;i++) if(Check(mask,i)==0) break;
    ...
    for( j=i+1;j<N;j++)
    {
        if(Check(mask,j)==0)
        {
            ret=min(ret,rec(mask|pre[i][j])+1);
        }
    }
    return ret;
}
    ...
    while(t--)
    {
        ...
        for(int i=0;i<N;i++)
        {
            for(int j=i+1;j<N;j++)
            {
                ...
                for(int k=0;k<N;k++)
                {
                    point a=MV(point(v[i].fs,v[i].sc),point(v[j].fs,v[j].sc));
                    point b=MV(point(v[i].fs,v[i].sc),point(v[k].fs,v[k].sc));
                    ll C=CP(a,b);
                    if(C==0) mask|=(1<<k);
                }
                pre[i][j]=mask;
