...
int main()
{
    ...
    while(t--)
    {
        ...
        {
            scanf("%c%d %c%d%*c",&c1,&a,&c2,&b);
//            deb(c1,c2,a,b);

            if(c1=='M') vL.pb(abc(a,b));
            else if(c1=='F')vR.pb(abc(b,a));
        }

        FOR(i,0,SZ(vL)+3) clr(G[i]);
        FOR(i,0,SZ(vL))
        {
            FOR(j,0,SZ(vR))
            {
//                deb(i,vL[i].Mid,vL[i].Fid);
//                deb(j,vR[j].Mid,vR[j].Fid);

                if(vL[i].Fid==vR[j].Fid  || vL[i].Mid==vR[j].Mid)
                {
//                    cout<<"As"<<endl;
                    G[i].pb(j);
                }
