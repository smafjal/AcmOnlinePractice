...
int N,M;
ll dis[MAX];
ll MM=2305843009213693952L;

void Bellman()
{
    for(int i=0;i<=N;i++)
    {
        dis[i]=2305843009213693952L;
    }
    ...
    for(int i=1;i<N;i++)
    {
        for(int j=0;j<SZ(v);j++)
        {
            int u=v[j].fs;
            int vv=v[j].sc;
            ll busy= (busyness[vv]-busyness[u]);
            busy=busy*busy*busy;

            if(dis[u]+busy<dis[vv] and dis[u]!=MM) dis[vv]=dis[u]+busy;
        }
    }
