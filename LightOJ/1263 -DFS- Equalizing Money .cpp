...
{
    col[s]=1;
    if(dis[s]<key) lower+=(key-dis[s]);
    else if(dis[s]>key) upper+=(dis[s]-key);
    for(int i=0;i<SZ(node[s]);i++)
    {
        if(!col[node[s][i]]) DFS(node[s][i]);
    }
    return;
}
