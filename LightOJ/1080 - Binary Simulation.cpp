...
int tree[4*MAX],col[4*MAX];
string base;char s[MAX];
int N,Q;
...
void update(int nod,int left,int right,int i,int j)
{
    ...
    update(nod*2,left,mid,i,j);
    update(nod*2+1,mid+1,right,i,j);
    tree[nod]=tree[nod*2]+tree[nod*2+1];
    return;
}
int quary(int nod,int left,int right,int i)
{
    if(col[nod]) refresh(nod,left,right);
    if(left==right) return tree[nod];

    int mid=(left+right)/2;
    int p=0;
    if(i<=mid) p=quary(nod*2,left,mid,i);
    else p=quary(nod*2+1,mid+1,right,i);
    return p;
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        base=s;
        N=SZ(base);
        for(int i=0;i<=4*N;i++) {tree[i]=col[i]=0;}
        scanf("%d",&Q);int x,y,z; char c;
        printf("Case %d:\n",++cas);
        for(int i=0;i<Q;i++)
        {
            scanf(" %c",&c);
