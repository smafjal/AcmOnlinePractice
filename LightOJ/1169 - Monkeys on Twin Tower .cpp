...
int m1[MAX],m2[MAX],c1[MAX],c2[MAX],N;
int dp[MAX][3];

int rec(int pos,int id)
{
//    deb(pos,id);lookalike;

    if(pos==N-1) return (id)?m2[pos]:m1[pos];
    ...
    if(id)
    {
        ret=min(rec(pos+1,id)+m2[pos],ret);
        ret=min(rec(pos+1,0)+c2[pos]+m2[pos],ret);
    }
    else
    {
        ret=min(rec(pos+1,id)+m1[pos],ret);
        ret=min(rec(pos+1,1)+c1[pos]+m1[pos],ret);
    }
    return ret;
}
...
int main()
{
    ...
    {
        scanf("%d",&N);
        FOR(i,0,N) scanf("%d",&m1[i]);
        FOR(i,0,N) scanf("%d",&m2[i]);
        FOR(i,0,N-1) scanf("%d",&c1[i]);
        FOR(i,0,N-1) scanf("%d",&c2[i]);
        mem(dp,-1);

        int cnt=min(rec(0,0),rec(0,1));
        printf("Case %d: %d\n",++cas,cnt);
    }
