...
int rec(int bari,int col)
{
    ...
    if(col==0)
    {
        ret=min(ret,rec(bari+1,1)+adj[bari][1]);
        ret=min(ret,rec(bari+1,2)+adj[bari][2]);
    }
    else if(col==1)
    {
        ret=min(ret,rec(bari+1,0)+adj[bari][0]);
        ret=min(ret,rec(bari+1,2)+adj[bari][2]);
    }
    else if(col==2)
    {
...
int main()
{
    ...
    while(t--)
    {
        scanf("%d",&N); for(int i=0;i<=N;i++) Clr(adj[i]); int x,y,z;
        ...
        mem(dp,-1);
        int ans=oo;
        ans=min(ans,rec(1,0)+adj[0][0]);
        ans=min(ans,rec(1,1)+adj[0][1]);
        ans=min(ans,rec(1,2)+adj[0][2]);
        printf("Case %d: %d\n",++cas,ans);
    }
    return 0;
}
