...
int print(int i,int j)
{
    ...
    p=0;
    if(a[i]==b[j]) p=(p+print(i+1,j+1)%M)%M;
    else
    {
        if(ret==rec(i+1,j)) p=(p+print(i+1,j)%M)%M;
        if(ret==rec(i,j+1)) p=(p+print(i,j+1)%M)%M;
        if(ret==rec(i+1,j+1)) p=(p-print(i+1,j+1)+M)%M;
    }
