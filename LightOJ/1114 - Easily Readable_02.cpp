...
struct trie
{
    int word;
    int place[52];
    trie(){ word=0;mem(place,-1);}
};

char str1[10006],str[109];
trie T[MAX];
int tree;
...
void build(int node,int pos)
{
    ...
    int now=findit(str[pos]);
    if(T[node].place[now]==-1)
    {
        T[node].place[now]=++tree;
    }
    int then=T[node].place[now];
    build(then,pos+1);
    return;
}

int findit(int node,int pos)
{
    if(str[pos]=='\0') return T[node].word;
    int now=findit(str[pos]);
    if(T[node].place[now]==-1) return 0;
    int then=T[node].place[now];
    return findit(then,pos+1);
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        {
            gets(str1);
            stringstream ss;ss<<str1; int cnt=1;
            while(ss>>str)
            {
                int l=strlen(str);
                if(l>2)sort(&str[1],&str[l-1]);
