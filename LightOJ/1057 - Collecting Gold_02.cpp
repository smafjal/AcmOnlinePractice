...
{
    if(mask==0) return max(Abs(v[cnt].fs-v[cur].fs),Abs(v[cnt].sc-v[cur].sc));

    int &ret=dp[cur][mask];
    if(vis[cur][mask]==cas) return ret;
    vis[cur][mask]=cas;
    ...
    for(int i=0;i<cnt;i++)
    {
        if((mask&(1<<i))!=0)
        {
            int dis=max(Abs(v[i].fs-v[cur].fs),Abs(v[i].sc-v[cur].sc));
    ...
    while(t--)
    {
        ...
        for(int i=0;i<R;i++)
        {
            for(int j=0;j<C;j++)
            {
                cin>>x;
                if(x=='x') {s1=i,s2=j;}
                if(x=='g') {v.pb(mp(i,j)),cnt++;}
            }
        }
        v.pb(mp(s1,s2));
        mem(dp,-1);
        int ans=rec(cnt,(1<<cnt)-1);
        printf("Case %d: %d\n",cas,ans);
    }
    return 0;
}
