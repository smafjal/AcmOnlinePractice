...
int  tree[4*MAX];
struct abc store[MAX+50004];
int N,Q;

int com(struct abc a,struct abc b)
{
    if(a.idx==b.idx)
    {
...
int main()
{
    ...
    while(t--)
    {
        ...
        for(int i=N+1,k=1;k<=Q;i++,k++)
        {
            /// Order for com() function define well.
            scanf("%d %d",&x,&y);
            store[i].idx=y;
            store[i].val=x;
            store[i].flag=i; /// (flag>N) means it's a Query type.
            store[i].pos=k;
        }

        mii M;
        sort(&store[1],&store[N+Q+1],com);
        int arr[50099];
