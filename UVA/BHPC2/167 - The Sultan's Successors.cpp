...
void rec(string sol)
{
//    deb("Sol:------------->> ",SZ(sol),sol);lookalike;
    if(SZ(sol)==8) {store.pb(sol);return;}
    FOR(i,1,9) if(isValid(sol,i)) rec(sol+(char)(i+'0'));
    return;
}
...
int main()
{
    ...
    while(t--)
    {
        FOR(i,1,9)FOR(j,1,9) cin>>grid[i][j];
        int maxl=0;
        FOR(i,0,SZ(store))
        {
            int cnt=0;
