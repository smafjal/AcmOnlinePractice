...
bool rec(int cur,int last,int take,int mask)
{
//    BINARY(mask);
//    deb(cur,last,take);lookalike;
    ...
    FOR(i,0,SZ(v))
    {
        ...
        if(v[i].fs==last) ok|=rec(i,v[i].sc,take+1,Set(mask,i));
        if(v[i].sc==last) ok|=rec(i,v[i].fs,take+1,Set(mask,i));
        if(ok) return ok;
    }
    return ok;
}
...
int main()
{
    while(cin>>n)
    {
        ...
        if(rec(0,v[0].fs,0,1<<0) || rec(0,v[0].sc,0,1<<0)) cout<<"YES"<<endl;
        else cout<<"NO"<<endl;
    }
    return 0;
}
