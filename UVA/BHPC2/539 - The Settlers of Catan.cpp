...
int rec(int mask,int last)
{
    ...
    {
        int v=adj[last][i];
        if(E.count(mp(last,v))>0) e=E[mp(last,v)];
        else if(E.count(mp(v,last))>0) e=E[mp(v,last)];
        if(!Check(mask,e)) ret=max(ret,rec(Set(mask,e),v)+1);
    }
    return ret;
}
...
int main()
{
    while(scanf("%d %d",&N,&M)==2)
    {
        ...
        FOR(i,0,M)
        {
            ...
            adj[x].pb(y);
            adj[y].pb(x);
            if(E.count(mp(x,y))<1 and E.count(mp(y,x))<1) E[mp(x,y)]=++cnt;
            v.pb(mp(x,y));
        }
        int ans=0;
        FOR(i,0,SZ(v))
        {
            ans=max(ans,rec((1<<E[v[i]]),v[i].sc)+1);
            ans=max(ans,rec((1<<E[v[i]]),v[i].fs)+1);
        }
        cout<<ans<<endl;
    }
    return 0;
