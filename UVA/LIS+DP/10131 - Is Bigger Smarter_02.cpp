...
bool com(Dpii a, Dpii b)
{
    if(a.sc.fs!=b.sc.fs) return a.sc.fs<b.sc.fs;
    return a.sc>b.sc;
}

void print(int idx)
{
    if(par[idx]!=-1) print(par[idx]);
    cout<<store[idx].fs+1<<endl;
}

void LIS(void)
{
    int n=SZ(store);
    FOR(i,0,n) L[i]=1,par[i]=-1;
    int maxl=0;int idx;

    FOR(i,0,n)
    {
        int w=store[i].sc.fs;
        int iq=store[i].sc.sc;
        FOR(j,0,i)
        {
            if(store[j].sc.fs<w and store[j].sc.sc>iq)
            {
                if(L[i]<=L[j])
                {
                    L[i]=L[j]+1;
