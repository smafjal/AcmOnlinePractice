...
//    deb(I[0]);lookalike;
    ...
    FOR(i,0,n)
    {
        ...
        {
            int mid=(low+high)/2;
//            deb("Arr:---->> ",store[mid],store[i]);lookalike;
...
//            deb("Bin::------->>>>> ",low);lookalike;
        }

//        deb(i,low);lookalike;
        I[low]=store[i];
        L[i]=low;
        if(maxl<low) maxl=low;
    }
...
//    lookalike;
    ...
    for(int j=i-1;j>=0;j--)
    {
        if(L[j]==L[i]-1 and store[j]<store[i])
        {
            res.pb(j);
            i=j;
        }
