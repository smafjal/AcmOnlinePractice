...
void pathprint(int s,int t,int id)
{
//    deb("P:--------->>>  ",s,t);lookalike;
    ...
    pathprint(s,path[s][t],id);
    pathprint(path[s][t],t,1);
    return;
}
...
int main()
{
    ...
    {
        scanf("%d %d",&S,&T);
        FOR(i,1,N+1)FOR(j,1,N+1) grid[i][j]=oo;

        FOR(i,0,E)
        {
            scanf("%d %d %lf %lf",a1+i,a2+i,R+i,D+i);
            grid[a1[i]][a2[i]]=min( grid[a1[i]][a2[i]],R[i]);
            grid[a2[i]][a1[i]]=min( grid[a2[i]][a1[i]],R[i]);
        }
        ...
        FOR(i,0,E)
        {
            if(R[i]<tapmatra+ERR)
            {
                grid[a1[i]][a2[i]]=min( grid[a1[i]][a2[i]],D[i]);
                grid[a2[i]][a1[i]]=min( grid[a2[i]][a1[i]],D[i]);
            }
        }
