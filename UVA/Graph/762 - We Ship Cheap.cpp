...
int main()
{
    ...
    while(scanf(" %d ",&N)==1)
    {
        ...
        {
            cin>>s1>>s2;
            if(M.count(s1)<1) M[s1]=SZ(M)+1;
            if(M.count(s2)<1) M[s2]=SZ(M)+1;
            adj[M[s1]].pb(M[s2]);
            adj[M[s2]].pb(M[s1]);
            store[M[s1]]=s1;
            store[M[s2]]=s2;
        ...
        cin>>s1>>s2;
        int t;
        if(M.count(s1)<1 and M.count(s2)<1 and s1==s2)
        {
            printf("\n");
            continue;
        }
        if(M.count(s1)<1 or M.count(s2)<1)
        {
            printf("No route\n");
            continue;
        }
