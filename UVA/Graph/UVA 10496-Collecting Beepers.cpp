...
int B;
vpii store;
int col[MAX][MAX],dis[13][MAX][MAX];
int dp[13][1<<10+5];

void bfs(int id,int st1,int st2)
{
    queue<int>Q;
    mem(col,0);
    Q.push(st1);
...
{
    if(mask==0) return dis[cur][s1][s2];
    int &ret=dp[cur][mask];
    if(ret!=-1) return ret;
    ret=oo;
    ...
    FOR(i,0,SZ(store))
    {
        if(Check(mask,i))
        {
            int cost=dis[cur][store[i].fs][store[i].sc];
            ret=min(rec(i,mask^(1<<i))+cost,ret);
        }
    }
    return ret;
}
