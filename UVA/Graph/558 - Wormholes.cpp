...
int Bell()
{
    ...
    FOR(i,0,N-1)
    {
        FOR(j,0,SZ(store))
        {
            if(dis[store[j].sc.fs]+store[j].fs<dis[store[j].sc.sc])
            {
                dis[store[j].sc.sc]=dis[store[j].sc.fs]+store[j].fs;
            }
        }
    }
    FOR(j,0,SZ(store))
    {
        if(dis[store[j].sc.fs]+store[j].fs<dis[store[j].sc.sc]) return 1;
    }
    return 0;
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        {
            scanf("%d %d %d",&x,&y,&t);
            store.pb(mp(t,mp(x,y)));
        }
        int id=Bell();
        if(id)printf("possible\n");
