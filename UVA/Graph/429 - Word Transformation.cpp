...
{
    if(abs(SZ(a)-SZ(b))>=2) return 0;
    int cnt=0;
    FOR(i,0,SZ(a))
    {
        if(i<SZ(b)) if(a[i]==b[i]) continue;
        cnt++;
        if(cnt>=2) return 0;
    }
    if(SZ(a)!=SZ(b)) cnt++;
...
int main()
{
    ...
    while(t--)
    {
        string a,b; Clr(store);
        while(cin>>a)
        {
            if(a=="*") break;
            store.pb(a);
        }
        FOR(i,0,SZ(store)+1) Clr(adj[i]);
        FOR(i,0,SZ(store))
        {
            FOR(j,i+1,SZ(store))
            {
                if(valid(store[i],store[j]))
                {
                    adj[i].pb(j);
                    adj[j].pb(i);
                }
        ...
        while(getline(cin,c) and !c.empty())
        {
            stringstream ss;ss<<c;ss>>a;ss>>b;
            int st=find(all(store),a)-store.begin();
            int cnt=bfs(st,b);
            cout<<a<<" "<<b<<" "<<cnt<<endl;
        }
    }
