...
{
    int leftshift=0,rightshift=0,top=0,down=0;
    for(int i=y;i<N;i++)
    {
        if(grid[x][i]=='x' or grid[x][i]=='@') {rightshift++;col[x][i]=ship;}
        else break;
    }

    for(int i=y;i>=0;i--)
    {
        if(grid[x][i]=='x' or grid[x][i]=='@') {leftshift++;col[x][i]=ship;}
        else break;
    }
    int vertical=rightshift+leftshift-1;

    for(int i=x;i<N;i++)
    {
        if(grid[i][y]=='x' or grid[i][y]=='@') {down++;col[i][y]=ship;}
        else break;
    }
    for(int i=x;i>=0;i--)
    {
...
int main()
{
    ...
    while(t--)
    {
        ...
        FOR(i,0,N)
        {
            FOR(j,0,N)
            {
                if(col[i][j]==0 and (grid[i][j]=='x' or grid[i][j]=='@'))
                {
                    ship++;
                    dfs(i,j);
                }
        ...
        FOR(i,0,N)FOR(j,0,N)
        {
            if(col[i][j] and grid[i][j]=='x')
            {
                if(!M[col[i][j]]) { M[col[i][j]]=1;cnt++;}
            }
        }
