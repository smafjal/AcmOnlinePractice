...
vector<int>G[MAX];
pair<double,double>nCor[MAX],mCor[MAX];
int N,M,S;
...
{
    return sqrt(((nCor[i].fs-mCor[j].fs)*(nCor[i].fs-mCor[j].fs))+((nCor[i].sc-mCor[j].sc)*(nCor[i].sc-mCor[j].sc)));
}
