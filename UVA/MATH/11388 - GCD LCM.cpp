...
void sive()
{
    ...
    for(int i=3;i<MAX;i+=2)
    {
        if(flag[i]==0)
        {
            prime.pb(i);
            int r=i*2;
            if(i<=MAX/i) for(int j=i*i;j<MAX;j+=r) flag[i]=1;
        }
    }
    return;
}
...
void factorize(int N,int id)
{
    for(int i=0;prime[i]<=sqrt(N);i++)
    {
        if(N%prime[i]==0)
        {
            ...
            if(id)M2[prime[i]]+=cnt;
            else M1[prime[i]]+=cnt;
        }
    }
    if(N>1) if(id)M2[N]+=1;else M1[N]+=1;
    return;
}
...
pair<int,int>makeit()
{
    ...
    FOREACH(it,M1)
    {
        int gp=(*it).fs;
        int gfre=(*it).sc;
        if(M2.count(gp)<1) return mp(-1,-1);
        int lfre=M2[gp];
        g*=power(gp,min(gfre,lfre));
        l*=power(gp,max(gfre,lfre));
    }
    FOREACH(it,M2)
    {
        int p=(*it).fs;
        if(M1.count(p)>0) continue;
        l*=power(p,M2[p]);
    }
    return mp(g,l);
}
...
int main()
{
    ...
    while(t--)
    {
        scanf("%d %d",&G,&L);  clr(M1);clr(M2);
        if(G==0 or L==0) {  cout<<-1<<endl;continue;}
        factorize(G,0);
        factorize(L,1);
        pair<int,int>ans=makeit();
        if(ans.fs!=-1)printf("%d %d\n",ans.fs,ans.sc);
        else printf("-1\n");
    }
    return 0;
}
