...
int rec(int i,int j)
{
    if(i>=SZ(v1) or j>=SZ(v2)) return 0;
    int &ret=dp[i][j];
    if(ret!=-1) return ret;

    ret=0;
    if(v1[i]==v2[j]) ret=rec(i+1,j+1)+1;
    else
    {
...
{
    if(i>=SZ(v1) or j>=SZ(v2)) return;
    int &ret=dp[i][j];

    if(v1[i]==v2[j] and ret==rec(i+1,j+1)+1)
    {
        v.pb(v1[i]);
        print(i+1,j+1);
    }
