...
{
    col[u]=1;
    for(int i=0;i<SZ(adj[u]);i++) if(col[adj[u][i]]==0) dfs1(adj[u][i]);
    order.pb(u);
}
...
{
    col[u]=1;
    for(int i=0;i<SZ(radj[u]);i++) if(col[radj[u][i]]==0) dfs2(radj[u][i]);
}
...
int main()
{
    ...
    while(scanf("%d %d",&n,&m)==2)
    {
        ...
        for(int i=0;i<n+1;i++) {clr(adj[i]);clr(radj[i]);}
        ...
        for(int i=0;i<m;i++)
        {
            ...
            adj[x].pb(y);
            radj[y].pb(x);
            if(id==2) {adj[y].pb(x); radj[x].pb(y);}
        }
        mem(col,0);clr(order);
        for(int i=1;i<=n;i++) if(col[i]==0) dfs1(i);
        mem(col,0);

        int cnt=0;
        for(int i=0;i<SZ(order) and cnt<2;i++)
        {
            int v=order[n-1-i];
            if(col[v]==0) {dfs2(v);cnt++;}
        }
        cout<<(cnt<2)<<endl;
    }
    return 0;
