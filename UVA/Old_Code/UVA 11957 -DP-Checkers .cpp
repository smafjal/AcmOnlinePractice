...
//    deb("R:--->>> ",r,c);lookalike;
    ...
    if(r==0 and c<N and c>=0) return 1;
    if(r<0 or r>N or c>=N) return 0;
    ...
    if(r-1>=0)
    {
        if(grid[r-1][c+1]!='B') ret=(ret+rec(r-1,c+1))%Mod;
        if(c-1>=0) if(grid[r-1][c-1]!='B') ret=(ret+rec(r-1,c-1))%Mod;
        if(r-2>=0)
        {
            if(grid[r-1][c+1]=='B' and grid[r-2][c+2]!='B') ret=(ret+rec(r-2,c+2))%Mod;
            if(c-2>=0)if(grid[r-1][c-1]=='B' and grid[r-2][c-2]!='B') ret=(ret+rec(r-2,c-2))%Mod;
        }
    }

//    if(r-2>=0) if(grid[r-1][c+1]=='B' and grid[r-2][c+2]!='B') ret=(ret+rec(r-2,c+2))%Mod;
//    else if(r-1>=0 and grid[r-1][c+1]!='B' ) ret=(ret+rec(r-1,c+1))%Mod;
//
//    if(r-1>=0)
//    {
//        if(r-2>=0 and c-1>=0 and ) if(grid[r-1][c-1]=='B' and grid[r-2][c-2]!='B') ret=(ret+rec(r-2,c+2))%Mod;
//        if(grid[r-1][c+1]!='B') ret=(ret+rec(r-1,c+1))%Mod;
//    }
    return ret;
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        FOR(i,0,N)
        {
            ...
            {
                cin>>grid[i][j];
                if(grid[i][j]=='W') px=i,py=j;
            }
        }

        mem(dp,-1);
        ll cnt=0;
        cnt=(cnt+rec(px,py))%Mod;
        printf("Case %d: %d\n",++cas,cnt);
    }
