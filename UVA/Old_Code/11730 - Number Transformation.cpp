...
void pre()
{
    ...
    for(i=4;i<MAX;i++)
    {
        ...
        if(n!=m and n>1) store[m].pb(n);
    }

    return;
}

int bfs(int sourse,int T)
{
    int i,j,k,v;queue<pair<int,int> >Q; int cnt=0; pair<int,int> u;
    ...
    {
        u=Q.front();Q.pop();
        if(u.fs==T) return u.sc;

        for(i=0;i<SZ(store[u.fs]);i++)
        {
            if(u.fs+store[u.fs][i]<=T and !col[u.fs+store[u.fs][i]])
            {
                col[u.fs+store[u.fs][i]]=1;
                Q.push(mp(u.fs+store[u.fs][i],u.sc+1));
            }
        }
    }
    return -1;
...
{
    sive();pre();
    int i,j,t,cas=0;int S,T;
    while(scanf("%d %d",&S,&T)==2 and(S and T))
    {
        mem(col,0);
        int ans=bfs(S,T);
