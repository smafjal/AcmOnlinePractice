...
void doit(int x,int y)
{
    ...
    FOR(i,0,4)
    {
        ...
        {
            if(grid[r][c]==border) {findborder=1;break;}
        }
    }
    if(findborder) grid[x][y]='#';
    else grid[x][y]=' ';
    ...
    FOR(i,0,4)
    {
        int r=rrr[i]+x;
        int c=ccc[i]+y;
        if(r>=0 and c>=0 and r<N and c<=M) doit(r,c);
    }
    return;
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        mem(col,0);
        N=M=0; bool fl;
        while(gets(grid[N]) and grid[N][0]!='_')
        {
            M=max(M,(int)strlen(grid[N]));
            int l=strlen(grid[N]);
            for(int i=0;i<l;i++)
            {
                if(grid[N][i]!='_' and grid[N][i]!=' ' and grid[N][i] !='*' and grid[N][i]!='#')
                {
                    border=grid[N][i];
                    break;
                }
        ...
        FOR(i,0,N)
        {
            ...
            for(int j=0;grid[i][j];j++)
            {
                if(grid[i][j]=='*') {doit(i,j);fl=0;break;}
            }
        }
