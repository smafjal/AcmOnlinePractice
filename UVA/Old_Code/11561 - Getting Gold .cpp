...
int ch(int si,int sj)
{
    if(grid[si][sj+1]=='T' or grid[si][sj-1]=='T' or grid[si+1][sj]=='T' or grid[si-1][sj]=='T') return 1;
    return 0;
}
...
void DFS(int si,int sj)
{
    ...
    for(int i=0;i<4;i++)
    {
        int u=rrr[i]+si;
        int v=ccc[i]+sj;
        if(!col[u][v] and grid[u][v]!='#' and grid[u][v]!='#')
        {
            if(grid[u][v]=='G') cnt++;
            DFS(u,v);
        }
