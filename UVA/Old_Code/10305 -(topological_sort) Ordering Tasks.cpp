...
void dfs(int start)
{
    tim++;
    col[start]=1;
    for(int i=0;i<SZ(node[start]);i++)
    {
        if(!col[node[start][i]]) dfs(node[start][i]);
    }
    tim++;
    finish_time.pb(mp(tim,start));
}
...
int main()
{
    ...
    while(scanf("%d %d",&n,&m)==2)
    {
        ...
        sort(reall(finish_time));
        for(i=0;i<SZ(finish_time);i++)
        {
            if(i)printf(" %d",finish_time[i].sc);
            else printf("%d",finish_time[i].sc);
        }
        printf("\n");
        reset(n+2);
    }
