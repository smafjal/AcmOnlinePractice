...
int N,wei[MAX],val[MAX],dp[1002][32],G[105],pe;

int knp(int cur,int w)
{
    if(cur==N or w==0) return 0;
    int &ret=dp[cur][w];
    if(ret!=-1) return ret;
    ret=0;
    if(wei[cur]>w) ret=knp(cur+1,w);
    else ret=max(ret,max(knp(cur+1,w-wei[cur])+val[cur],knp(cur+1,w)));
    return ret;
}
