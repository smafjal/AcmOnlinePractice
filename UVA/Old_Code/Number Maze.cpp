...
#define MAX 1000
int row,cul;
ll grid[MAX][MAX],col[MAX][MAX],dis[MAX][MAX];
...
bool cheack(int x,int y)  {return (x>=1 and x<=row and y>=1 and y<=cul);}

bool relux(int u,int v,int x,int y,int cost)
{
    if(dis[u][v]+grid[x][y]<dis[x][y])
    {
        dis[x][y]=dis[u][v]+grid[x][y];
        return 1;
    }
    return 0;
}
...
//        }
        ...
        for(int i=0;i<4;i++)
        {
            ...
            if(cheack(x,y))
            {
                if((dis[x][y]==-1) or (dis[u][v]+grid[x][y]<dis[x][y]))  ///
                {
//                    deb("XY:  ",x,y);
                    dis[x][y]=dis[u][v]+grid[x][y];
