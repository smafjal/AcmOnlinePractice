...
int N,M;
ll dp[105][10][(1<<10)+1];
ll vis[105][10][(1<<10)+1];
int loop=1;
ll MO= 1000000007L;

ll rec(int num,int last,int mask)
{
//    deb(num,last);
//    BINARY(mask);
//    lookalike;
    ...
    ll &ret=dp[num][last][mask];
    if(vis[num][last][mask]==loop) return ret;
    vis[num][last][mask]=loop;

    ret=0;
    int now=mask;
    if(last+1<N)
    {
        if((now&(1<<(last+1)))==0) now=now|(1<<(last+1));
        ret+=rec(num-1,last+1,now^(1<<(last+1)));
        if(ret>=MO) ret=ret%MO;
    }
    now=mask;
    if(last-1>=0)
    {
