...
int rec(int x,int y,int idx,int mask)
{
//    deb(x,y,idx);lookalike;

    if(idx+1==SZ(a)) return 1;
    int ret=0;
    ...
    FOR(i,0,8)
    {
        ...
        {
            int u=r*4+c;
            if(grid[r][c]==a[idx+1] and !(mask&(1<<u))) ret|=rec(r,c,idx+1,mask^(1<<u));
            if(ret) return ret;
        }
    }
    return ret;
...
bool findit()
{
    FOR(i,0,4)FOR(j,0,4)if(grid[i][j]==a[0])
    {
        int mask=0;int u=i*4+j;
        mask^=(1<<u);
        int id=rec(i,j,0,mask);
        if(id) return 1;
    }
    return 0;
}
...
int main()
{
    ...
    while(N--)
    {
        FOR(i,0,4)FOR(j,0,4)scanf(" %c",&grid[i][j]);
        scanf(" %d",&M);
        ...
        FOR(i,0,M)
        {
            ...
            if(id)
            {
                if(SZ(a)==3 or SZ(a)==4) cnt++;
                else if(SZ(a)==5) cnt+=2;
                else if(SZ(a)==6) cnt+=3;
                else if(SZ(a)==7) cnt+=5;
                else cnt+=11;
            }
        }
