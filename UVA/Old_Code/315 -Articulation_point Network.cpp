...
vi adj[MAX];
int N;
int ti[MAX],low[MAX],col[MAX],artic[MAX];
int counte;

void make(int r,string s)
{
    stringstream ss;
    ss<<s;int k;
    while(ss>>k)
...
{
    col[u]=1;low[u]=ti[u]=counte++;int subtree=0;
...
{
    mem(ti,-1);mem(low,-1);mem(col,0);mem(artic,0);
    ...
    int cnt=0;
    for(int i=0;i<=N;i++) if(artic[i]) cnt++;
    return cnt;
}

int main()
{
    int x,y; char s[10000];
    while(scanf("%d",&N)==1)
    {
        if(N==0) break;
