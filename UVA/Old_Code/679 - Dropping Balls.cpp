...
//    deb("nod:--->>> ",nod);lookalike;
    ...
    {
        n1=tree[nod*2];
        if(nod*2+1<=(1<<n)-1) n2=tree[nod*2+1];

        if(n1==n2) {fl=1;build(nod*2);}
        else if(n1<n2) {fl=1;build(nod*2);}
        else {fl=1;build(nod*2+1);}
//        deb("t1 and t2:-->>> ",t1,t2);lookalike;
    }

    if(fl==0){no=nod;tree[nod]=1;}
}

//void view()
//{
//    FOR(i,1,1<<n) deb(i,tree[i]);
//}
