...
int  Bin(int left,int right,int x)
{
    ...
    {
        if(x<v[0]) {id=-1;return 0;}
        if(x>v[SZ(v)-1]) {id=-2;return SZ(v)-1;}
        if(v[right]<x and x<v[left]) return right;
    }
}
