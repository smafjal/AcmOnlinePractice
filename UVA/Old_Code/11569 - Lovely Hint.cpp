...
int rec(int cur,int last)
{
    ...
    ret=0;
    int d=base[cur]-'A'+1;
    if(last*5<=d*4) ret=max(ret,rec(cur+1,base[cur]-'A'+1)+1);
    ret=max(ret,rec(cur+1,last));
    return ret;
}

int find(int cur,int last)
{
    if(cur>=SZ(base)) return 1;
    int &ret=way[cur][last];
    if(ret!=-1) return ret;

    ret=0;
    int d=base[cur]-'A'+1;
    if(last*5<=d*4 and rec(cur,last)==rec(cur+1,base[cur]-'A'+1)+1) ret+=find(cur+1,base[cur]-'A'+1);
    if(rec(cur,last)==rec(cur+1,last)) ret+=find(cur+1,last);
    return ret;
}
...
int main()
{
    ...
    while(t--)
    {
        string str;
        char arr[300];
        str.clear();base.clear();

        scanf(" %[^\n]",arr);
        str=arr;
        for(int i=0;i<SZ(str);i++)
        {
            if(isUpperCase(str[i])) base+=str[i];
        }
        set<char>S(all(base));
        string s(all(S));
        base=s;
