...
struct abc
{
    int left,right,space,idx;
    abc(){;}
    abc(int _l,int _r,int _s,int _id){ left=_l; right=_r; space=_s;idx=_id;}
};
...
int main()
{
    while(scanf("%d",&N)==1)
    {
        ...
        FOR(i,0,N)
        {
            int l=0,r=0,sp=0;  bool fl=1;
            gets(grid[i]);
            FOR(j,0,25)
            {
                if(fl and grid[i][j]=='X') l++;
                else if(grid[i][j]==' ') {sp++,fl=0;}
                else r++;
            }
            v.pb(abc(l,r,sp,i));
        }
