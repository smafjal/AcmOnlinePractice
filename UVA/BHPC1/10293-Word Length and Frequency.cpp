...
int main()
{
    string str,s1,s; vs v; vs tmp;

    char arr[100]; int cas=0;
    while(scanf(" %[^\n]",arr)==1)
    {
        str=arr;
        if(str=="#")
        {
            ...
            for(int i=0;i<SZ(v);i++)
            {
                ...
                {
                    if(s1[j]==' ' or s1[j]=='!' or s1[j]=='?' or s1[j]==',' or s1[j]=='.')
                    {
                        if(!s.empty())
                        {
                            M[SZ(s)]++;
...
//            lookalike;
            ...
            FOR(i,0,SZ(st))
            {
                cout<<st[i].fs<<" "<<st[i].sc<<endl;
            }
            puts("");
            clr(v); clr(tmp);clr(st);
        }
        else v.pb(str);
    }
    return 0;
