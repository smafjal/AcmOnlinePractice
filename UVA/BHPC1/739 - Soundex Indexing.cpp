...
int main()
{
    ...
    FOR(i,0,SZ(v))
    {
        ...
        FOR(j,1,SZ(str))
        {
            ...
            {
                if(M[str[j-1]]==M[str[j]] and arr[M[str[j]]]) continue;
                arr[M[str[j-1]]]=0;

                arr[M[str[j]]]=1;
                if(SZ(s1)<4)s1+=(M[str[j]]+'0');
                else break;
            }
        }
        for(int j=SZ(s1);j<4;j++) s1+='0';

//        deb("S1:----->>> ",s1);lookalike;

        cout<<"         "<<v[i];
        for(int j=10+SZ(v[i]);j<=34;j++)
        {
            printf(" ");
        }
        cout<<s1<<endl;
