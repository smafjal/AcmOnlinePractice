...
double doit(pair<double,double> a,pair<double,double> b)
{
...
//    deb("M:--->>>",M);
    ...
    queue<int>Q;
    int col[MAX];mem(col,0);
    int dis[MAX];mem(dis,0);
...
int main()
{
    while(scanf("%lf %lf",&valo,&M)==2)
    {
        if(valo==0 and M==0) break;
        M=(M*60);
        FOR(i,0,MAX) clr(adj[i]);clr(store);
        string s;
        stringstream ss;

        double x1,x2,y1,y2;
        scanf("%lf %lf %lf %lf ",&sx1,&sy1,&tx1,&ty1);
        store.pb(mp(sx1,sy1));
        store.pb(mp(tx1,ty1));
        ...
        {
            if(s.empty()) break;
            ss<<s; ss>>x1;ss>>y1; clr(ss);
            store.pb(mp(x1,y1));
        }
//
//        FOR(i,0,SZ(store))
//        {
//            deb(store[i].fs,store[i].sc);lookalike;
//        }
        ...
        FOR(i,0,SZ(store))
        {
            ...
            {
                if(i==j) continue;
                double dis=doit(store[i],store[j]);
//                deb("dis:-->>> ",dis);lookalike;
                adj[i].pb(mp(j,dis));
            }
        }
