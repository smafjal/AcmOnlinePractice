...
int main()
{
    string s; int arr[102]; int cas=0;
    int t;
    scanf("%d",&t);
    while(t--)
    {
        ...
        {
            if(s[i]=='.') continue;
            if(s[i]=='>') {idx++;if(idx>=100)idx=0;}
            else if(s[i]=='<'){ idx--;if(idx<0)idx=99;}
            else if(s[i]=='+') {arr[idx]++;if(arr[idx]>255) arr[idx]=0;}
            else if(s[i]=='-') {arr[idx]--;if(arr[idx]<0)arr[idx]=255;}
        }
        printf("Case %d:",++cas);
        for(int i=0;i<100;i++)
        {
